...
void mainImage(out vec4 o, vec2 C) {
  ...
  for (vec2 r = iResolution.xy, Q; ++i < 60.;) {
    vec3 p = z*normalize(vec3(C-.5*r,r.y)); 
    p.z -= 4.; 
    vec3 S = p;
    d = p.y-T;
    ...
    // Calculate single intensity value instead of color vector
    float wave = sin(S.y+p.z*.5+S.z-length(S-p));
    intensity += (1.0 + wave) / d;
  }
  
  intensity = tanh(intensity / 1e4);
  o.xyz = vec3(intensity);
}
...
void main() {
  ...
  float intensity = o.r; // Single intensity value
